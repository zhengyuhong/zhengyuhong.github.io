...
如果序列长度是 $N$，$QK^\top$ 就是 $N\times N$ 的关系矩阵。它既带来 $O(N^2)$ 的时间开销，也需要保存与序列长度平方增长的中间注意力矩阵，长上下文因此很快变得昂贵。
...
论文用正值 feature map $\phi$ 替换原始指数相似度的一种可结合形式。核心变形可以写成：
...
左边需要先形成 $N\times N$ 的矩阵；右边先得到较小的 $\phi(K)^\top V$ 汇总状态，再让 $\phi(Q)$ 查询它。论文实验采用 $\phi(x)=\mathrm{elu}(x)+1$，以保证相似度为正并保持归一化稳定。
...
当前查询通过 $\displaystyle \frac{\phi(Q_i)^\top S_i}{\phi(Q_i)^\top Z_i}$ 读取状态并产生输出。训练时整段序列仍可并行；自回归推理时，每一步只更新固定大小的 $S_i$ 与 $Z_i$，这正是论文称其“像 RNN”的原因。
...
1. **瓶颈在 $QK^\top$**：标准 Attention 显式构造 $N\times N$ 关系矩阵，长序列的时间和显存因此呈平方增长。
2. **结合律带来固定状态**：通过 $\phi(Q)(\phi(K)^\top V)$ 和两个前缀状态 $S/Z$，因果推理可以用 RNN 式递推完成。
